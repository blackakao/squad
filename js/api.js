...
async function uploadImageAsset(kind, dataUrl, name = "image") {
  ...
  if (!response.ok) {
    throw new Error(`이미지 저장 실패: HTTP ${response.status} ${responseText}`);
  }

  const result = JSON.parse(responseText);
  if (!result?.path) {
    throw new Error("이미지 저장 응답에 path가 없습니다.");
  }
